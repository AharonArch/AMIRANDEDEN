...
const { FACTS, MEETINGS, WHATSAPP } = require('./_projectData');
  ...
  try{
    ...
    const r = await fetch('https://api.anthropic.com/v1/messages', {
      ...
      body: JSON.stringify({
        ...
        max_tokens: 1024,
        // The knowledge base is large and identical on every call, so we cache it:
        // first call pays full price, repeat calls within ~5 min read it at ~10% cost.
        system: [
          { type: 'text', text: buildKB(), cache_control: { type: 'ephemeral' } }
        ],
        messages
      })
    });
